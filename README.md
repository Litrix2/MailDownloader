...
## 运行
- 对于源代码文件,首先需要将 littoolkit 模块置于主程序同一目录下,请留意 littoolkit 版本是否匹配.  
- 对于可执行文件则没有特殊要求.
...
如果你想选择不同的文件来加载,则需要在终端中打开程序并输入选项:
    ...
    mail_downloader.py -c your_config.toml

或

    mail_downloader.exe -c your_config.toml

- 选项 -c 为配置文件路径,如果是相对路径则相对于工作目录.
    - 配置中的各个下载路径同理.
...
## 1.4.2
- **注意**
    - **过滤器配置逻辑已改变,请前往示例配置文件查看信息.**

- 改进: 改进三个过滤器的关系
- **修复: imap服务器登出导致后续邮件无法获取的问题**
- 修复: 某些邮箱文件夹无法选择的问题
- 修复: 邮件过滤后处理总数不会增加的问题
- 修复: 尝试获取无发送时间的邮件时引发异常的问题
- 修复: 将邮件作为附件转发时附件无法正常下载的问题
- 优化: 代码优化
- 其他问题修复
